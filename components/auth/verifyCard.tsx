  ...
  async function onSubmit(data: VerifyFormValues) {
    startTransition(async () => {
      try {
        ...
        const result = await verifyCode(userId!, verificationCode, token!);
        console.log(result, "verifyCode result");

        if (result.success) {
          toast.success("Successfully verified!");
          const { email, password, rememberMe } = result.credentials;
                ...
                >
                  {[0, 1, 2, 3, 4, 5].map((index) => (
                    <Input
                      ...
                      type="text"
                      maxLength={1}
                      className="w-14 h-14 bg-white/5 border-white/10 text-white text-center text-xl font-semibold rounded-xl  focus:ring-2 focus:ring-purple-500 focus:border-white transition-all duration-200"
